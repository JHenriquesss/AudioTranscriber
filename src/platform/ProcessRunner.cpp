...
shared::Result<ProcessOutput> ProcessRunner::run(const ProcessSpec &spec) const {
    ...
    auto stdoutFuture = std::async(std::launch::async, readPipeToEnd, stdoutRead);
    auto stderrFuture = std::async(std::launch::async, readPipeToEnd, stderrRead);
    ...
    if (waitResult == WAIT_TIMEOUT) {
        ...
        output.exitCode = 1;
        output.stdoutText = stdoutFuture.get();
        output.stderrText = stderrFuture.get();
        CloseHandle(stdoutRead);
        CloseHandle(stderrRead);
