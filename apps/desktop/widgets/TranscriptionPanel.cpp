...
void TranscriptionPanel::setAvailableModels(const std::vector<app::ModelDisplayEntry> &models) {
    modelCombo_->clear();
    for (const app::ModelDisplayEntry &model : models) {
        modelCombo_->addItem(QString::fromStdString(model.name), QString::fromStdString(model.id));
    }
}
